    ...
    <div
        ...
        <div class="w-full mb-1">
            <div class="mb-4">
                    ...
                    <ol class="inline-flex items-center space-x-1 text-sm font-medium md:space-x-2">
                        ...
                        <li>
                            <div class="flex items-center">
                                ...
                                <a href="{{ url('admin/ruang/all') }}"
                                    class="ml-1 text-gray-700 hover:text-primary-600 md:ml-2 dark:text-gray-300 dark:hover:text-white">Ruang</a>
                            </div>
                        </li>
                        <li>
                            <div class="flex items-center">
                ...
                <h1 class="text-xl font-semibold text-gray-900 sm:text-2xl dark:text-white">Semua Ruang</h1>
            </div>
            <div class="sm:flex">
                <div class="items-center hidden mb-3 sm:flex sm:divide-x sm:divide-gray-100 sm:mb-0 dark:divide-gray-700">
                    <form class="lg:pr-3" action="{{ route('admin.ruang.index') }}" method="GET">
                        <label for="ruang-search" class="sr-only">Search</label>
                        <div class="relative mt-1 lg:w-64 xl:w-96">
                            <input type="text" name="name" id="ruang-search"
                                class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
                                placeholder="Cari nama ruang" value="{{ request('name') }}">
                        </div>
                    </form>
                </div>
                <div class="flex items-center ml-auto space-x-2 sm:space-x-3">
                    <button type="button" data-modal-target="add-ruang-modal" data-modal-toggle="add-ruang-modal"
                            ...
                            @foreach ($ruang as $room)
                                ...
                                <tr class="hover:bg-gray-100 dark:hover:bg-gray-700">
                                    <td class="p-4 text-base font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                        {{ $id++ }}</td>
                                    <td class="p-4 text-base font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                        {{ $room->nama_ruang ?? 'N/A' }}</td>
                                    <td class="p-4 text-base font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                        {{ $room->jadwalPelajaran->first()->kelas->nama_kelas ?? 'N/A' }}</td>
                                    <td class="p-4 text-base font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                        {{ $room->jadwalPelajaran->first()->pelajaran->nama_pelajaran ?? 'N/A' }}</td>
                                    <td class="p-4 text-base font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                        {{ $room->jadwalPelajaran->first()->guru->user->name ?? 'N/A' }}</td>
                                    <td class="p-4 space-x-2 whitespace-nowrap">
                                        <button type="button"
                                            data-modal-target="detail-ruang-modal{{ $room->id }}"
                                        ...
                                        <button type="button"
                                            data-modal-target="edit-ruang-modal{{ $room->id }}"
                                        ...
                                        <button type="button"
                                            data-modal-target="delete-ruang-modal{{ $room->id }}"
