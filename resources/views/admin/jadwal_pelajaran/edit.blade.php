@foreach ($jadwal_pelajaran as $jadwal)
    ...
    <div class="fixed left-0 right-0 z-50 items-center justify-center hidden overflow-x-hidden overflow-y-auto top-4 md:inset-0 h-modal sm:h-full"
        id="edit-jadwal_pelajaran-modal{{ $jadwal->id }}">
                    ...
                    <form action="{{ route('admin.jadwal_pelajaran.update', $jadwal->id) }}" method="POST"
                        ...
                        <div class="grid grid-cols-6 gap-6">
                            ...
                            <div class="col-span-6 sm:col-span-3">
                                ...
                                <input type="time" value="{{ old('jam_mulai', $jadwal->jam_mulai ?? 'N/A') }}" name="jam_mulai" id="jam_mulai"
                                    class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
                                    required>
                            </div>
                            <div class="col-span-6 sm:col-span-3">
                                <label for="jam_selesai" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Jam Selesai</label>
                                <input type="time" value="{{ old('jam_selesai', $jadwal->jam_selesai ?? 'N/A') }}" name="jam_selesai" id="jam_selesai"
                                    class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
                                    required>
                            </div>
                            <div class="col-span-6 sm:col-span-3">
                                <label for="pelajaran_id"
                                ...
                                <select name="pelajaran_id" id="pelajaran_id"
                                    class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
                                    required>
                                    @foreach ($lesson as $student)
                                        <option value="{{ $student->id }}">{{ $student->nama_pelajaran }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-span-6 sm:col-span-3">
                            ...
                            <div class="col-span-6 sm:col-span-3">
                                ...
                                <select name="ruang_id" id="ruang_id"
                                    ...
                                    @foreach ($room as $student)
                                        <option value="{{ $student->id }}">{{ $student->nama_ruang }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
